...
export default function CheckoutPage({ nguoiDung, dangXuat }) {
  ...
  const getLoggedRole = (vaiTro) => {
    const role = (vaiTro || '').toLowerCase();
    if (role.includes('quản lý') || role.includes('quan ly') || role === 'quanly') return 'quanly';
    if (role.includes('kế toán') || role.includes('ke toan') || role === 'ketoan') return 'ketoan';
    return 'sale';
  };
  ...
  return (
    <KhungNhanVien nguoiDung={nguoiDung} dangXuat={dangXuat}>
      <CheckoutContainer
        ...
        setCheDoNhanVien={() => {}}
        chuyenTrang={(trang) => {
          if (trang === 'staff_reception') navigate(ROUTES.dashboard);
        }}
        loggedRole={getLoggedRole(nguoiDung?.vaiTro)}
      />
